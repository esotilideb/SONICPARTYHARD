...
class CheckboxThingie extends FlxSprite
{
	...
	public function new(x:Float = 0, y:Float = 0, ?checked = false) {
		...
		frames = Paths.getSparrowAtlas('mainmenu/box');
		animation.addByPrefix("unchecked", "cross", 24, false);
		animation.addByPrefix("unchecking", "cross", 24, false);
		animation.addByPrefix("checking", "check", 24, false);
		animation.addByPrefix("checked", "check", 24, false);
	...
	override function update(elapsed:Float) {
		if (sprTracker != null) {
			setPosition(sprTracker.x + 800 + offsetX, sprTracker.y + offsetY);
			if(copyAlpha) {
