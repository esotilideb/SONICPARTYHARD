...
class PlayState extends MusicBeatState
{
	...
	public static var STRUM_X_MIDDLESCROLL = STRUM_X;
	...
	public static var restartSelected:Bool = false;
	...
	override public function create()
	{
		...
		timeTxt.setFormat(Paths.font("SUPEB__.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		timeTxt.scrollFactor.set();
		timeTxt.alpha = 0;
		timeTxt.borderSize = 2;
		timeTxt.visible = updateTime = showTime;
		if(ClientPrefs.data.downScroll) timeTxt.y = FlxG.height;
		...
		healthBar = new Bar(150, (!ClientPrefs.data.downScroll ? 75 : 740), 'health', function() return health, 0, 2);
		...
		scoreTxt = new FlxText(200, (!ClientPrefs.data.downScroll ? 50 : 720), FlxG.width, "", 20);
		scoreTxt.setFormat(Paths.font("SUPEB__.ttf"), 28, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		scoreTxt.scrollFactor.set();
		scoreTxt.borderSize = 1.25;
		scoreTxt.visible = !ClientPrefs.data.hideHud;
		uiGroup.add(scoreTxt);

		miscTxt = new FlxText(200, (!ClientPrefs.data.downScroll ? 210 : 870), FlxG.width, "placeholder", 20);
		miscTxt.setFormat(Paths.font("SUPEB__.ttf"), 28, FlxColor.WHITE, LEFT, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		miscTxt.scrollFactor.set();
		miscTxt.borderSize = 1.25;
		miscTxt.visible = !ClientPrefs.data.hideHud;
		updateScore(false);
		uiGroup.add(miscTxt);

		botplayTxt = new FlxText(400, timeBar.y + 55, FlxG.width - 800, "- BOTPLAY -", 32);
		botplayTxt.setFormat(Paths.font("SUPEB__.ttf"), 32, FlxColor.WHITE, CENTER, FlxTextBorderStyle.OUTLINE, FlxColor.BLACK);
		...
		jumpscare = new FlxSprite(0, 425);
		jumpscare.frames = Paths.getSparrowAtlas('end');
		jumpscare.screenCenter();
		jumpscare.antialiasing = false;
		jumpscare.scale.set(3, 3);
		jumpscare.visible = false;
		jumpscare.cameras = [camHUD];
		jumpscare.animation.addByPrefix('end', 'end', 24, true);	
		if (ClientPrefs.data.flashing)	jumpscare.animation.play('end');
		else trace("cancelled! flashing lights disabled!");
		uiGroup.add(jumpscare);	
		...
		if (!seenCutscene) {	
			switch(SONG.song) { 
				case 'ultra-sonic-blast': 
					startVideo('introsonic');   
			}	
		}

		if (!inCutscene)	startCallback();
		else startCountdownScripts();
	...
	public var alrSet:Bool = false;
	public function startCountdownScripts()
	{
		if (alrSet == false) {
	...
	public function startCountdown()
	{
		...
		if(ret != LuaUtils.Function_Stop) {
			...
			if (alrSet == false)	startCountdownScripts();
	...
	public dynamic function updateScore(miss:Bool = false)
	{
		...
		var tempMisc:String = 'Misses: ${songMisses} | Acc: ${str}';
		// "tempScore" variable is used to prevent another memory leak, just in case
		// "\n" here prevents the text from being cut off by beat zooms
		scoreTxt.text = '${tempScore}\n';
		miscTxt.text = '${tempMisc}\n';
	...
	public function doScoreBop():Void  return;
	...
	{
		var strumLineX:Float = ClientPrefs.data.middleScroll ? STRUM_X_MIDDLESCROLL : STRUM_X + 100;
		var strumLineY:Float = ClientPrefs.data.downScroll ? (FlxG.height - 200) : 80;
		for (i in 0...4)
		{
			// FlxG.log.add(i);
	...
	function resetRPC(?showTime:Bool = false) return;
	...
	{
		trace("seen cutscene:" + seenCutscene);
		...
		if(!endingSong && !inCutscene && allowDebugKeys)
		{
			if (controls.justPressed('debug_1'))
				...
				jarvisHazmeUnBotplayInvaluable();
		}
	...
	var botplayCreated:Bool = false;
	function jarvisHazmeUnBotplayInvaluable()
	{
		if (botplayCreated) {
			trace(':vin:');
			//botplayCreated = false;
			botplayTxt.visible = false;
	...
	public function triggerEvent(eventName:String, value1:String, value2:String, strumTime:Float) {
		...
		switch(eventName) {
			...
			case 'Jumpscare': //sorry...
				
				if (value1 == "true" || value1 == "visible" || value1 == "show") jumpscare.visible = true;
				else if (value1 == "false" || value1 == "invisible" || value1 == "hide") jumpscare.visible = false;
	...
	private function popUpScore(note:Note = null):Void
	{
		...
		for (i in seperatedScore)
		{
			...
			numScore.x = placement + (43 * daLoop) - 90 + 535;
			numScore.y = 535;
			trace(numScore.y);
			...
			FlxTween.tween(numScore, {alpha: 1}, 0.5, {
		...
		FlxTween.tween(rating, {alpha: 1}, 0.5, {
			//startDelay: Conductor.crochet * 0.001 / playbackRate
		});

		FlxTween.tween(comboSpr, {alpha: 0}, 0.5, {
			onComplete: function(tween:FlxTween)
			{
				comboSpr.destroy();
				rating.destroy();
